...
public class RedisExpirationListenerServices {
    ...
    public void startListening() {
        // Get all cluster nodes and print them first
        redisClusterClient.getPartitions().forEach(partition ->
                System.out.println("Node: " + partition.getUri() + " | ID: " + partition.getNodeId() + " | Role: " + partition.getRole().name())
        );

        // Filter only MASTER nodes and construct RedisURI list
        List<RedisURI> clusterNodes = redisClusterClient.getPartitions().stream()
                .filter(partition -> "UPSTREAM".equalsIgnoreCase(partition.getRole().name())) // Only select MASTER nodes
                .map(partition -> {
        ...
        System.out.println("Subscribing to Redis Cluster MASTER nodes: " + clusterNodes);

        // Enable keyspace notifications on all MASTER nodes
        clusterNodes.forEach(this::enableKeyspaceNotifications);

        // Subscribe to key expiry events on each MASTER node
